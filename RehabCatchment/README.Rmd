...
if (!("tidyverse" %in% ip | "sf" %in% ip | "units" %in% ip |
      "tmaptools" %in% ip))
    stop ("This script requires the 'tidyverse' 'sf', 'units', and 'tmaptools' ",
...
coordinate reference system is retained.
...
to only those within a specified distance of the rehab locations, chosen here as
10km. Note that we just use straight line distances here, because we only need
to roughly determine which postcodes surround our rehab centres. The subsequent
calculations will then use more accurate distances along street networks.
...
Select random addresses using a geocoded database, available via the `PSMA`
package.
```{r PSMA-install, eval = FALSE}
if (!"PSMA" %in% installed.packages() [, 1])
    devtools::install_github("HughParsonage/PSMA")
```
We then specify how many random addresses we want to sample per postcode. These
addresses will provide the points ultimately used to determine case loads for
the three rehab centres.
```{r addressesPerPostcode}
addressesPerPostcode <- 1000
```

We then write a special function so we can sample the postcodes as we go.
...
into `sf::POINT` objects. (This function takes a few seconds because of the
`fetch_postcodes` call.)
```{r random-addresses}
library(PSMA)
...
many points can be quite slow using `tmap` or `mapview`, so if you want to view
the results, you might need to be patient. (Much faster plotting can be achieved
...
```{r tmap-postcode-polygons, eval = FALSE}
tmap_mode("view")
...
```{r mapview-postcode-polygons, eval = FALSE}
...
Computing the road distance and travel time from each address to each hospital
first requires a local copy of the street network within the bounding polygon
defined by `basicDemographicsRehab`. This is easiest done with the `dodgr`
package, which directly calls the `osmdata` package to do the downloading.
...
`dodgr` package needs to de-compose the `sf`-formatted street network, which
...
```
The results can be viewed with the usual 3 approaches, with both `tmap` and
`mapview` first requiring these points to be converted to `sf` form. This in
turn requires first extracting the coordinates as a simple numeric matrix.
```{r pts-to-sfc, eval = FALSE}
...
time.)
```{r catchment-polygons, message = FALSE, eval = FALSE}
bounding_polygon <- sf::st_transform(basicDemographicsRehab,
                                     sf::st_crs(4326)) %>%
  sf::st_union () 
...
Then plot with `tmap` (with which it is not trivial to overlay points marking
rehab centres):
```{r catchment-polygon-plot-tmap, eval = FALSE}
tmap_mode("view")
tm_shape(v) +
...
```{r catchment-polygon-plot-mapview, eval = FALSE}
mapview (v) %>%
...
Finally, we need a per postcode breakdown of proportion of addresses going to
each centre, so that we can compute the number of cases going to each centre.
The above procedure occasionally mapped multiple addresses onto the same network
...
postcodes
```
```{r postcode-groups-kable, eval = TRUE, echo = FALSE}
postcodes <- data.frame (POSTCODE = randomaddresses$POSTCODE,
                         DestNumber = DestNumber,
                         Destination = BestDestination,
                         stringsAsFactors = FALSE) %>%
...
converting to our sample size of `addressesPerPostcode = ` `r
addressesPerPostcode`. The following code also removes previous demographic
columns (the `select` line).
...
This then just needs to be mapped back on to our `postcodes` data of number
allocated to each rehab centre from each postcode.
```{r join-postcodes-to-demographics}
...
postcodes
```
We then just need to repeat the previous code, multiplying the estimated `total`
by our new `stroke_rate` variable.
