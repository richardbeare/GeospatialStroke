...
tools for this.
```{r catchment-display-prep}
# Note - quite a few NAs in the d matrix - MP to check
# Perhaps the road network based on the convex hull misses something?
#
# This time around I'll return a value of 4 for the NAs so
# we can see what is going on when we plot it
DestNames <- c(rownames(RehabLocations), "Unknown")
# assign each source address to the nearest destination
DestNumber <- apply(d, MARGIN=1, 
                    function(Y) {
                      res <- which.min(Y)
                      if (length(res)==0) res <- 4
                      return(res)
                    })

BestDestination <- DestNames[DestNumber]
## Each address now has a destination associated with it, so we could plot points on the map
## and infer the catchment basin based on colour. However it is nicer to construct a polygon
## representation of the catchment basin, as it displays nicely and we can compute areas etc.

## approach here is to create Voronoi tesselation of the random addresses, so that each address is associated
## with a polygon containing it. The polygon encloses the area that is closer to the address than any other.

fromCoords <- nodes[fromIDX, ]
fromCoords$Destination <- BestDestination
## Some of the entries in d are duplicated - address map to the same osm node?
## filter out the repeated entries
fromCoords <- fromCoords[!duplicated(fromCoords$id),]

## plot on tmap - this looks sensible
library(tmap)
tmap_mode("view")
fromCoords_sf <- st_as_sf(fromCoords, coords=c("x", "y"))
tm_shape(fromCoords_sf) + tm_dots(col="Destination") + 
  tm_basemap("OpenStreetMap")

## filter unique fromCoords
fromCoords_unique <- distinct(fromCoords, id, .keep_all=TRUE)
fromCoords_unique_sf <- st_as_sf(fromCoords_unique, coords=c("x", "y"))

## create an st object
g <- st_multipoint(as.matrix(fromCoords_unique[,c("x", "y")]))
...
## st_voronoi doesn't return the polygons in the same order as the original point, which is very annoying.
vorder <- unlist(st_intersects(fromCoords_unique_sf, v))
v <- v[vorder]
Kingston <- st_union(st_sf(v[fromCoords_unique$Destination=="KingstonHospital"], crs=4326))
Kingston <- st_intersection(Kingston, bounding_polygon)

Unknown <- st_union(st_sf(v[fromCoords_unique$Destination=="Unknown"], crs=4326))
Unknown <- st_intersection(Unknown, bounding_polygon)

Casey <- st_union(st_sf(v[fromCoords_unique$Destination=="CaseyHospital"], crs=4326))
Casey <- st_intersection(Casey, bounding_polygon)

Dandenong <- st_union(st_sf(v[fromCoords_unique$Destination=="DandenongHospital"], crs=4326))
Dandenong <- st_intersection(Dandenong, bounding_polygon)

catchments <- st_sf(geometry=c(Dandenong, Casey, Kingston, Unknown), Destination=c("Dandenong", "Casey", "Kingston", "Unknown"))

# This looks sensible now
# remind me to make a PR for sf, as the reordering of voronoi tesselations is a pain if you want to do anything other than
# plotting it.
library(tmap)
tmap_mode("view")
tm_shape(catchments) + tm_polygons(col="Destination", alpha=0.5) + 
  tm_basemap("OpenStreetMap")
