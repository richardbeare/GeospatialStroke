...
To stay within limits, and have enough spare, I'm going to sample 10,000 of those random addresses for each rehab centre.
...
res1 <- lapply(r1, function(x) {
  ...
  js <- gsub(" ","",js)
  fn <- paste0("~/Documents/github/GeospatialStroke/data/googleway/directions_queries/rehab1/",x,"_results.json")
  f <- file(fn)
  writeLines(js, f)
  close(f)
  return( js )
...
res2 <- lapply(r2, function(x) {
  ...
  js <- paste0(js, collapse = "")
  js <- gsub(" ","",js)
  fn <- paste0("~/Documents/github/GeospatialStroke/data/googleway/directions_queries/rehab2/",x,"_results.json")
  f <- file(fn)
  writeLines(js, f)
  close(f)
  return( js )
...
res3 <- lapply(r3, function(x) {
  js <- googleway::google_directions(
    ...
    js <- paste0(js, collapse = "")
  js <- gsub(" ","",js)
  fn <- paste0("~/Documents/github/GeospatialStroke/data/googleway/directions_queries/rehab3/",x,"_results.json")
  f <- file(fn)
  writeLines(js, f)
  close(f)
  return( js )
...
directions1 <- lapply( files, jsonlite::fromJSON )



lst_files <- list.files(path = paste0(path, "/rehab2/") )
files <- paste0(path, "/rehab2/", lst_files)

directions2 <- lapply( files, jsonlite::fromJSON )

lst_files <- list.files(path = paste0(path, "/rehab3/") )
files <- paste0(path, "/rehab3/", lst_files)

directions3 <- lapply( files, jsonlite::fromJSON )
...
df1 <- data.frame(
  polyline = sapply( directions1, googleway::direction_polyline )
  , distance_m = sapply( directions1, get_distance )
  , duration_s = sapply( directions1, get_duration )
  )
