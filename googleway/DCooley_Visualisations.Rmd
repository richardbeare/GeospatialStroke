...
get_directions <- function(rehab) {
  ...
  ## TODO( read directions, including file path 'x' value so we know the r1 value)
...
get_polyline <- function(x) {
  googleway::direction_polyline( x )
}
check_status <- function(x) {
  x[["status"]] == "OK"
}

format_directions <- function( d ) {
  lst <- lapply( directions1, function(x){
    res <- x[["result"]]
    row <- x[["sample_row"]]
    secs <- ifelse( check_status( res ), get_duration( res ), NA_real_ )
    dist <- ifelse( check_status( res ), get_distance( res ), NA_real_ )
    polyline <- ifelse( check_status( res ), get_polyline( res ), NA_character_ )
    data.frame(
      sample_row = as.numeric( row )
      , distance_m = dist
      , duration_s = secs
...
#   # polylines <- sapply( d, function(x) googleway::direction_polyline( x[["result"]] ) )
#   data.frame(
#     # sample_row = as.numeric( sample_row )
#     # , polyline = polylines
#      distance_m = dist
#     , duration_s = secs
#     #, duration_m = round( secs / 60, 2 )
...
Note inside the foramtting function I have a `check_status()` function. This is to check the status of the Google query. Sometimes you will get a "OVER_QUERY_LIMIT", or "REQUEST_DENIED" value, indicating there was a problem with the query. 
